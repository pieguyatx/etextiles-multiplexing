...
void loop() {
  ...
  int periodMax = map(ambientLight, 0, 1023, 12, 0);
    
  // set timing MAXIMUM, over all LEDs. Bias toward longer period when darker ambient light.
  int period = random(0.2*periodMax, periodMax); // seconds
  if (debug==true){
    Serial.print("Period = ");
...
//  timeshift = timeshift*random(70,100)/100;  // skew distribution hack, instead of uniform distribution
  ...
  for(count6=0; count6<sizeRow; count6++){
    for(count3=0; count3<sizeCol; count3++){
      ...
      ledPeriod[count6][count3] = random(0, period); // seconds
  ...
  while(loopdone == false)
  {
    ...
    for(count6 = 0; count6<sizeRow; count6++)  // go row by row
    {
      for(count3 = 0; count3<sizeCol; count3++) // go col by col
      {
        ...
        if( timenow - timecyclestart <= (ledPeriod[count6][count3]*1000 + ledShift[count6][count3]) ) // time < end of ledPeriod
        {
          if( timenow - timecyclestart >= ledShift[count6][count3] ) // time > start of period
          {
            ...
            for (count3=0; count3<sizeCol; count3++){
              ...
              brightness[count6][count3] = round( (timesegments-2)*0.5*(
                  1 - cos( coeff*(timenow-timecyclestart-ledShift[count6][count3])/1000 )) + 2);                 
