...
//  }
  ...
  int periodMax = map(ambientLight, 0, 1023, 1000*maxcycletime, 500);  // last arguments set output range; milleseconds
//  if (debug==true){
...
//  }
  ...
  int period = random(0.5*periodMax, periodMax); // milliseconds
//  if (debug==true){
//    Serial.print("Period = ");
...
//  }
  ...
  for(count6=0; count6<sizeRow; count6++){
    for(count3=0; count3<sizeCol; count3++){
      ...
      ledPeriod[count6][count3] = random(0.1*period, period); // milliseconds
      // Set time shifts for each LED.
      ledShift[count6][count3] = random(0, period - ledPeriod[count6][count3]); // milliseconds
    }
  }
  ...
  while(loopdone == false)
  {
    ...
    timePassed = timenow - timecyclestart;
    if (timePassed < 0){
    ...
    for(count6 = 0; count6<sizeRow; count6++)  // go row by row
    {
      for(count3 = 0; count3<sizeCol; count3++) // go col by col
      {
            ...
            timePassed >= ledShift[count6][count3] ) // time > start of period
        {
          ...
          brightness[count6][count3] =  (timesegments-ledMinBrightness[count6][count3])*0.5*(
...
//    //DEBUG^^ 
    ...
    if( timePassed > (period + 500) ) // milliseconds
    {
      loopdone = true;
    }
